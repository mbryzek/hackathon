<script lang="ts">
  ...
  const variantClasses = {
    ...
    secondary: 'bg-gray-700 text-white hover:bg-gray-800 focus:ring-gray-500 disabled:bg-gray-400',
    ghost: 'bg-transparent text-gray-700 hover:bg-gray-100 shadow-none focus:ring-gray-400 disabled:text-gray-400',
  ...
  const cursorClass = $derived(disabled || loading ? 'cursor-not-allowed opacity-70' : 'cursor-pointer');
...
{#if href && !disabled}
  <a {href} class={buttonClasses} target={external ? '_blank' : undefined} rel={external ? 'noopener noreferrer' : undefined}>
    {#if loading}
...
{:else}
  <button class={buttonClasses} {onclick} disabled={disabled || loading} type="button">
    {#if loading}
      <svg class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
        <path

<script lang="ts">
  ...
  let { variant = 'default', padding = 'md', hover = false, header, footer, children }: Props = $props();
  ...
  const cardClasses = $derived(`rounded-lg overflow-hidden ${variantClasses[variant]} ${hoverClass}`);

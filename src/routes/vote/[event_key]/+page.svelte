<script lang="ts">
    ...
    async function handleSubmit() {
        ...
        await goto(
            `${urls.voteThanks(eventKey)}?code=${encodeURIComponent(code)}`,
        );
    }
    ...
    const canSubmit = $derived(
        verification &&
            ...
            selectedProjectIds.size <= (verification?.max_votes || 0),
    );
        ...
        <div class="space-y-6">
                ...
                >
                    <div>
                        ...
                        <p class="text-gray-600 mt-1">
                            Voting as: <span class="font-semibold"
                                >{voterTypeLabel}</span
                            >
                        </p>
                    </div>
                    <div
                        class="bg-yellow-100 text-yellow-800 px-4 py-2 rounded-lg text-center"
                    >
            ...
            <div class="space-y-4">
                {#each verification.projects as pv (pv.project.id)}
                    ...
                    {@const isDisabled =
                        !isSelected &&
                        selectedProjectIds.size >= verification.max_votes}
                    <button
                        type="button"
                        ...
                        class="w-full text-left bg-white shadow rounded-xl p-6 transition-all duration-200
							...
							{isDisabled && verification.max_votes > 1

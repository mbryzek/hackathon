...
{#snippet sponsorCard(sponsor: (typeof sponsorsY26)[number], isLead: boolean)}
  ...
  {#if sponsor.url}
    ...
    >
      <img src={sponsor.logo} alt={sponsor.name} class="w-full h-full object-contain p-4" />
    </a>
  {:else}
    <div class={baseClass}>
      <img src={sponsor.logo} alt={sponsor.name} class="w-full h-full object-contain p-4" />
...
<Shell title="2026 Sponsors">
  <p class="text-gray-800 font-light mb-8">A huge thank you to our sponsors who make the 2026 Hackathon possible!</p>

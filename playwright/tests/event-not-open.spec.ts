...
test.describe("Event Gating", () => {
  test("missing event shows not-available on voting page", async ({
    page,
  }) => {
  ...
  test("missing event shows not-available on thanks page", async ({
    page,
  }) => {
    await helpers.loadUrl(
      page,
      `/vote/does-not-exist-event-key/thanks?code=${UNVERIFIED_CODE}`,
    );
    await expectNotAvailable(page);
  });

  test("draft event shows not-available on voting page", async ({
    apiHelpers,
    page,
  }) => {
  ...
  test("draft event shows not-available on thanks page", async ({
    apiHelpers,
    page,
  }) => {
    const testEvent = await createEventWithStatus(apiHelpers, EventStatus.Draft);
    ...
    await helpers.loadUrl(
      page,
      `/vote/${testEvent.event.key}/thanks?code=${code}`,
    );
    await expectNotAvailable(page);
  });

  test("closed event shows not-available on voting page", async ({
    apiHelpers,
    page,
  }) => {
  ...
  test("closed event shows not-available on thanks page", async ({
    apiHelpers,
    page,
  }) => {
    const testEvent = await createEventWithStatus(apiHelpers, EventStatus.Closed);
  ...
  test("open event renders the voting UI (not the gate)", async ({
    apiHelpers,
    page,
  }) => {
  ...
  test("open event renders the thanks UI (not the gate)", async ({
    apiHelpers,
    page,
  }) => {
    const testEvent = await createEventWithStatus(apiHelpers, EventStatus.Open);
